...
namespace TestVulkan
{
	...
	public class VulkanTutorial
	{
		...
		private string MODEL_PATH = Asd.Directory + @"\TestVulkan\models\viking_room.obj";
		private string TEXTURE_PATH = Asd.Directory + @"\TestVulkan\textures\viking_room.png";
		...
		*/
		...
		{
			byte[] vertShaderCode = Help.ReadFile(Asd.Directory + @"\TestVulkan\shaders\vert.spv");
			byte[] fragShaderCode = Help.ReadFile(Asd.Directory + @"\TestVulkan\shaders\frag.spv");
		...
		unsafe private void CreateImageView(Image image, Format format, ImageAspectFlags aspectFlags, uint mipLevels, ref ImageView imageView )
		{
			...
			fixed (ImageView* imageViewL = &imageView)
			{
